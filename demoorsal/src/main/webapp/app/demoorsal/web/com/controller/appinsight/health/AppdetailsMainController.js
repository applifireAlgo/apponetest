Ext.define('Demoorsal.demoorsal.web.com.controller.appinsight.health.AppdetailsMainController', {
     extend: 'Demoorsal.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AppdetailsMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AppdetailsTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#AppdetailsForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#AppdetailsGrid');
               var tree = this.view.down('#AppdetailsTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#AppdetailsForm');
          var grid = this.view.down('#AppdetailsGrid');
          var tree = this.view.down('#AppdetailsTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#AppdetailsGrid');
               ...
               var formPanel = this.getView().up().down('#AppdetailsForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#AppdetailsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AppdetailsForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AppdetailsGrid').store;
          gridStore.removeAll();
