...
@RestController
@SourceCodeAuthorClass(createdBy = "deepali.arvnd@algorhythm.co.in", updatedBy = "", versionNumber = "1", comments = "Service for PasswordAlgo Master table Entity", complexity = Complexity.LOW)
...
public class PasswordAlgoServiceImpl extends PasswordAlgoService {
    ...
    public HttpEntity<ResponseBean> findById(@RequestBody FindByBean findByBean) throws SpartanPersistenceException, Exception {
        ...
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        try {
            loadcheck.app.shared.aaaboundedcontext.authentication.PasswordAlgo lstpasswordalgo = passwordAlgorepo.findById((java.lang.String) findByBean.getFindKey());
            responseBean.add("success", true);
            responseBean.add("message", "Successfully retrived ");
            responseBean.add("data", lstpasswordalgo);
        } catch (org.springframework.transaction.TransactionException e) {
