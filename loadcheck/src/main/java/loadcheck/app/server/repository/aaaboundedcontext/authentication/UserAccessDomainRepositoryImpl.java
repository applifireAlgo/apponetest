...
@Repository
@SourceCodeAuthorClass(createdBy = "deepali.arvnd@algorhythm.co.in", updatedBy = "", versionNumber = "1", comments = "Repository for UserAccessDomain Master table Entity", complexity = Complexity.LOW)
public class UserAccessDomainRepositoryImpl extends SearchInterfaceImpl implements UserAccessDomainRepository<UserAccessDomain> {
    ...
    public List<UserAccessDomain> findAll() throws SpartanPersistenceException {
        try {
            ...
            java.util.List<loadcheck.app.shared.aaaboundedcontext.authentication.UserAccessDomain> query = emanager.createQuery("select u from UserAccessDomain u where u.systemInfo.activeStatus=1").getResultList();
            return query;
    ...
    public List<UserAccessDomain> save(List<UserAccessDomain> entity) throws SpartanPersistenceException, SpartanConstraintViolationException {
        try {
            ...
            for (int i = 0; i < entity.size(); i++) {
                loadcheck.app.shared.aaaboundedcontext.authentication.UserAccessDomain obj = entity.get(i);
    ...
    public void delete(String id) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            loadcheck.app.shared.aaaboundedcontext.authentication.UserAccessDomain s = emanager.find(loadcheck.app.shared.aaaboundedcontext.authentication.UserAccessDomain.class, id);
            emanager.remove(s);
        } catch (javax.persistence.PersistenceException e) {
    ...
    public UserAccessDomain findById(String userAccessDomainId) throws Exception, SpartanPersistenceException {
        try {
            ...
            loadcheck.app.shared.aaaboundedcontext.authentication.UserAccessDomain listOfUserAccessDomain = (loadcheck.app.shared.aaaboundedcontext.authentication.UserAccessDomain) query.getSingleResult();
